...
public class SentrixMobileContainerTaskCreatePlugin implements ContainerTaskCreatePlugin {
    ...
    private void resortContainerTasks(List<ContainerTaskDTO> containerTasks, ContainerTaskTypeEnum containerTaskType, List<Long> newCustomerTaskIds) {
        List<Long> workingWorkStationIds = workStationApi.queryAllWorkStation().stream()
                .filter(v -> WorkStationStatusEnum.ONLINE == v.getWorkStationStatus())
                .filter(v -> OperationTaskTypeEnum.PICKING == v.getOperationType())
                .map(WorkStationDTO::getId).toList();
        ...
        Map<String, List<OutboundPlanOrderDTO>> customWaveMap = outboundPlanOrderApi.findAllByStatus(List.of(OutboundPlanOrderStatusEnum.NEW)).stream().collect(Collectors.groupingBy(OutboundPlanOrderDTO::getCustomerWaveNo));

        // 空闲槽口不为空，并空闲槽口的数量比新波次数量多
        if (!CollectionUtils.isEmpty(idlePutWallSlots) && idlePutWallSlots.size() > customWaveMap.size()) {
    ...
    private void resortContainerTasksForIdle(List<ContainerTaskDTO> containerTasks, ContainerTaskTypeEnum containerTaskType, List<Long> newCustomerTaskIds) {
        ...
        List<ContainerTaskDTO> allContainerTasks = containerTaskApi.queryContainerTaskListAndExcludeContainerTaskTypes(ContainerTaskStatusEnum.processingStates, List.of(BusinessTaskTypeEnum.PICKING), List.of(ContainerTaskTypeEnum.TRANSFER)).stream()
                .filter(task -> !staticContainerCodes.contains(task.getContainerCode())).toList();
        if (CollectionUtils.isEmpty(allContainerTasks)) {
            log.info("All container tasks are completed");
            return;
        }

        Set<Long> operationTaskIds = allContainerTasks.stream()
                .flatMap(task -> task.getRelations().stream()).map(ContainerTaskAndBusinessTaskRelationDTO::getCustomerTaskId).collect(Collectors.toSet());
        ...
        // 过滤掉实际已经完成的搬箱任务
        allContainerTasks = allContainerTasks.stream()
                .filter(task -> task.getRelations().stream().anyMatch(relation -> uncompletedOperationTaskIds.contains(relation.getCustomerTaskId()))).toList();
        ...
        Map<String, Optional<Integer>> containerOrderPriorityMap = allContainerTasks.stream()
        ...
        // 所有未完成订单需要的货架
        Map<String, Set<Long>> containerCompleteOrders = allOperationTaskDTOS.stream()
                .collect(Collectors.groupingBy(OperationTaskDTO::getSourceContainerCode, Collectors.mapping(OperationTaskDTO::getOrderId, Collectors.toSet())));

        Map<Long, Set<String>> orderRequiredContainers = allOperationTaskDTOS.stream()
                .collect(Collectors.groupingBy(OperationTaskDTO::getOrderId, Collectors.mapping(OperationTaskDTO::getSourceContainerCode, Collectors.toSet())));

        // 所有未完成货架可以满足的订单行
        Map<String, Set<Long>> containerCompleteLines = allOperationTaskDTOS.stream()
                .collect(Collectors.groupingBy(OperationTaskDTO::getSourceContainerCode, Collectors.mapping(OperationTaskDTO::getDetailId, Collectors.toSet())));

        Map<Boolean, List<ContainerTaskDTO>> containerTaskMap = allContainerTasks.stream()
                .collect(Collectors.groupingBy(t -> containerOrderPriorityMap.get(t.getContainerCode()).isEmpty()
                        || containerOrderPriorityMap.get(t.getContainerCode()).get() == 0));
        stopWatch.stop();
